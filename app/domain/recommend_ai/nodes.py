...
## 추천 규칙
## 추천 규칙
1. 카드/보험/정책 각각 반드시 최소 1개, 최대 {MAX_RECOMMEND}개 선택 (하나라도 0개면 심각한 감점)
2. 추천 사유 작성 규칙:
   - 반드시 유저의 실제 소비 데이터(금액, 카테고리)를 언급할 것
   - 지식그래프(Neo4j) 근거가 있는 상품을 추천할 경우, 이를 활용하여 상품이 유저의 소비 카테고리와 어떻게 연결되는지 자연스럽게 설명할 것
   - 예시: "이번 달 식비 {top3[0]['amount']:,}원을 지출했는데, 이 카드로 매달 약 XX원 절약 가능해요"
   - 친근하고 응원하는 톤으로 작성 (딱딱한 설명 금지)
   - 2~3문장으로 작성하되, 첫 문장은 유저 소비 패턴 언급, 두 번째 문장은 상품 혜택 설명, 세 번째 문장은 예상 절약 금액이나 혜택 강조
   - "~할 것 같습니다" 표현 절대 금지 → "~할 수 있어요", "~에 딱 맞아요" 등 사용
   - 숫자로 구체적인 혜택 금액이나 할인율을 포함할 것 (예: "월 최대 3만원 할인", "10% 캐시백")
...
### 보험 추천 사유 예시
- "20대 초반에는 건강하더라도 갑작스러운 사고나 입원에 대비하는 게 중요해요. 이 보험은 월 보험료가 부담 없으면서 입원일당 3만원, 수술비 최대 500만원을 보장해줘서 사회초년생에게 딱 맞아요!"
- "여행을 자주 다니시나요? 이 보험은 해외여행 중 발생하는 의료비, 배상책임, 분실까지 폭넓게 보장해줘요. 여행 전 꼭 챙겨두면 든든할 거예요 ✈️"

### 정책 추천 사유 예시  
- "나이와 소득 조건이 딱 맞는 청년 지원 정책이에요! 매달 최대 XX만원을 지원받을 수 있어서 생활비 부담을 크게 줄일 수 있어요. 신청 마감이 얼마 안 남았으니 서둘러보세요!"
- "취업 준비 중이시라면 이 정책을 꼭 확인해보세요. 교육비와 취업 활동비를 지원해줘서 자기계발에 집중할 수 있어요. 조건이 맞으면 바로 신청하는 걸 추천해요 💪"
