...
# LLM Judge (100점 만점)
# =============================================
def llm_judge_score(user, results) -> float:
    client = anthropic.Anthropic()
    age    = datetime.now().year - user["profile"]["birth"].year
    income = user["profile"]["monthly_income"]
    summary = sorted(user["monthly_summary"], key=lambda x: x["amount"], reverse=True)
    spending_text = "\n".join([f"  - {s['category']}: {s['amount']:,}원 ({s['ratio']:.0f}%)" for s in summary])

    def fmt(items, label):
        if not items: return f"{label}: 없음"
        lines = [f"{label}:"]
        for i, item in enumerate(items, 1):
            benefit = item.get("core_benefit") or item.get("top_benefit","")
            lines.append(f"  [{i}] {item['name']} - {benefit}")
            lines.append(f"      추천사유: {item['reason'][:60]}")
        return "\n".join(lines)
...
## 평가 기준
1. 소비 패턴과 추천 상품의 혜택 일치도 (40점)
2. 추천 사유가 유저 소비 데이터를 구체적으로 언급했는가 (30점)
3. 나이/소득 조건을 만족하는 정책이 추천됐는가 (20점)
4. 중복 없이 다양한 상품이 추천됐는가 (10점)
...
def run_stage(stage, user_embeddings, conn, vconn):
    ...
    for idx, user in enumerate(TEST_USERS):
        ...
        if ok:
            if idx < len(TEST_USERS) - 1:
                print(f"✅  ({CALL_INTERVAL}초)", flush=True)
    ...
    for user in TEST_USERS:
        if user["name"] in failed: continue
        ...
        print(f"      Cosine={cosine:.4f}  Faith={faith:.4f}  ILD={ild:.4f}  LLM={llm_score}/100")
...
def main():
    ...
    print("📊 단계별 성능 비교")
    print("="*62)
    print(f"{'스테이지':<30} {'Cosine':>8} {'Faith':>8} {'ILD':>8} {'LLM/100':>8}")
    print("-"*62)
    for r in results:
        print(f"{r['stage']:<30} {r['cosine']:>8.4f} {r['faithfulness']:>8.4f} {r['ild']:>8.4f} {r['llm_judge']:>8.1f}")
    print("="*62)
